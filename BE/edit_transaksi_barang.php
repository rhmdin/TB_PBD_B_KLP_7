...
session_start(); 
    ...
    if( $show ["id"] > 0 ){        
            ...
            $sub_total = (($qty *  $jual) + $ongkir - $diskon);
                    ...
                    $Tanggal = date('Y-m-d');	 
                    $update =  pg_query($connect, "UPDATE tabel_detail_transaksi SET diskon = $diskon, ongkir = $ongkir, kuantitas_barang = $qty  WHERE no_invoice = '$noInvoice' AND id_barang = '$idBarang'");
                    if($update){
                        echo "<script>
